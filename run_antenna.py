#!/usr/bin/env python3
"""
Run electromagnetic antenna simulation.

This script solves the Maxwell-Helmholtz antenna problem where the antenna
radiates electromagnetic waves from a prescribed excitation field.
"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Electromagnetic antenna simulation using NGSolve"
    )
    ...
    parser.add_argument(
        ...
        metavar=("X", "Y", "Z"),
        help="Antenna excitation polarization direction (default: 1 0 0)"
    )
    parser.add_argument(
        "--amplitude", type=float, default=1.0,
        help="Antenna excitation amplitude (default: 1.0)"
    )
    ...
    parser.add_argument(
        "--antenna-radius", type=float, default=0.1,
        help="Antenna radius (default: 0.1)"
    )
    parser.add_argument(
...
def main():
    ...
    if not args.quiet:
        print("=" * 60)
        print("Electromagnetic Antenna Simulation")
        print("=" * 60)
    ...
    if not args.quiet:
        ...
        print(f"  Excitation polarization: {args.polarization}")
        print(f"  Excitation amplitude: {args.amplitude}")
    ...
    E_source = create_antenna_source(
    ...
    # Setup problem
    if not args.quiet:
        print(f"\nSetting up antenna problem...")
    problem = AntennaProblem(mesh, k, E_source, fes_order=args.order)
    ...
    if args.output:
        ...
        results = {
            ...
            "parameters": {
                ...
                "wavenumber": k,
                "excitation_polarization": args.polarization,
                "excitation_amplitude": args.amplitude,
                "outer_radius": args.outer_radius,
                "antenna_radius": args.antenna_radius,
