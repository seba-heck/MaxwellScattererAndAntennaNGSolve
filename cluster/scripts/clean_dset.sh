#!/bin/bash
# CLEAN-UP DATASET
# A little bash script to control the produced folders and files from the simulations
# and to move the important files into the right folders. It has a function 'draw_progress'
# for a large, nice progress bar. And does following jobs:
#  - Iterate through all subfolders of given 'SOURCE_DIR'.
#  - Check if all required files from the list 'REQUIRED_FILES' are available.
#  - Rename and move the drawn HTML-files into a separate folder 'visu'.
#  - Move all 'REQUIRED_FILES' from this sample to new subfolder in 'TARGET_DIR'.
#
# USAGE: clean_dset.sh SOURCE_DIR TARGET_DIR  
