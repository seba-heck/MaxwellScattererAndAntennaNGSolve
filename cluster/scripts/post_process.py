...
def main():
    ...
    if len(error_jobs["meshing"]) > 0 or len(error_jobs["error"]) > 0 or len(error_jobs["other"]) > 0:
        ...
        fails_dict = {'meshing': " - Meshing failed in {} jobs: {}",
                      'error':   " - Errors in {} jobs: {}",
                      'segment': " - Segmentation fault in {} jobs: {}",
                      'time':    " - Time limit reached in {} jobs: {}",
                      'other':   " - Other error msg in {} jobs: {}"}
        
        print("\nFound failed jobs:")

        for fail_ in fails_dict:
            if len(error_jobs[fail_]) > 0:
