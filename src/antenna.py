"""
Antenna problem formulation.

This module implements the antenna radiation problem where the antenna
emits/radiates electromagnetic waves. The difference from scattering is
that the source is the antenna itself rather than an external incident wave.
"""
...
class AntennaProblem:
    """
    Electromagnetic antenna problem using Maxwell-Helmholtz formulation.

    This class models an antenna that radiates electromagnetic waves.
    The antenna is excited by a prescribed field E_source on its surface,
    and the problem solves for the radiated field in the surrounding space.

    The formulation is similar to ScattererProblem, but instead of an
    external incident wave, the excitation comes from the antenna itself.

    Attributes:
        mesh: NGSolve mesh with PML regions
        k: Wavenumber (2π/λ)
        E_source: Antenna excitation field (tangential field at antenna surface)
    ...
    def __init__(
        ...
        """
        Initialize the antenna problem.

        Args:
            mesh: NGSolve mesh with PML regions and named boundaries
            k: Wavenumber (2π/λ)
            E_source: Antenna excitation field (electric field at antenna surface)
    ...
    def setup_finite_element_space(self):
        ...
        print(f"HCurl FEM space created for antenna problem:")
    ...
    def assemble_system(self):
        """
        Assemble the bilinear and linear forms for the antenna problem.
        ...
        where E_source is the antenna excitation field at the surface.
        The impedance boundary condition allows radiation into the PML.
        """
        if self.fes is None:
        ...
        # Impedance boundary condition on antenna surface (allows radiation)
        # ∫_Γ -ik (n×E)·(n×E') dΓ
        ...
        # Antenna excitation boundary term
        # ∫_Γ -ik (n×E_source)·(n×E') dΓ
        self.l += -1j * self.k * Cross(n, self.E_source) * Ep.Trace() * ds("inner")

        print("Antenna system assembled")
